...
Daily operator commands (build, appliance update, extension reload, tests): [`docs/COMMANDS.md`](docs/COMMANDS.md).

## Windows installation

For a public-release installation on Windows, follow the Turkish mini guide: [`docs/WINDOWS_MINI_KURULUM.md`](docs/WINDOWS_MINI_KURULUM.md).
...
Control Center: http://127.0.0.1:8000/login (`admin@psbi.local` / `password` local-only). Search at `/control/search` with URL `https://prod.x.com/kasko` and query `XML`. Wiki wizard and pause/resume live under `/control/sources`.
